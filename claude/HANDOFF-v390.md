...
- **THE "hands-free" problem — RESOLVED in v391 (the real cause was NOT printing).** After a long detour through PrintNode, Spencer finally pinned it: "nothing pops up, I just have to click **the actual print/Ship button by the service** — on prod it flips to *printing/shipped* on its own, on sandbox it doesn't." That auto-fire is **`autoBookOnShip`** (auto-books a rule-matched order → the Ship button presses itself). It had been **removed from the Ship dropdown earlier this session** at Spencer's request ("get rid of the dropdown option book and print automatically"), so sandbox literally lost the control. **v391 restores it** as the 4th Ship-dropdown option ("Book & print it automatically — no click"). Awaiting Spencer's confirmation (he was testing tomorrow). NOTE: `autoBookOnShip` only fires for orders that match an **Autopilot rule** (`matched.src==="autopilot"`) — if sandbox has no matching rules it still won't auto-fire; that's a data thing, not code.
- The printing-pipeline fixes from v384–v390 (below) are all still valid and good hardening (tiny-corner, blank/half, retry-with-original, prefix strip, the Diagnose button) — they just weren't the actual cause of "I have to click the button."
