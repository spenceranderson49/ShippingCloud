  ...
  try {
    ...
    if (action === "requestCompanyAdmin" || String(action).startsWith("company")) {
      ...
      if (action === "companySetFlags") {
        ...
        const DEPLOY_KEYS = { _custom: 1, _products: 1, _addresses: 1, _companyLogo: 1, _companyLogoLock: 1, _boxes: 1, _boxesLock: 1, _fieldLists: 1, _fieldListsLock: 1, _docs: 1, _docsLock: 1 };
        const flags = {}; let n = 0;
    ...
    if (action === "putMany") {
      ...
      if ("users" in toWrite) {
        const cur = await getStore("users");
        /* PASSWORD-SAFETY (critical): the client's copy of `users` carries NO password hashes —
           stripUsers() removes them before they ever leave the server. We re-attach each hash from
           the CURRENT server value in mergeUsersForWrite below. If this read FAILED (Supabase
           timeout/network blip — most likely right after a deploy or on a cold start), merging
      ...
      if ("featureFlags" in toWrite && toWrite.featureFlags && typeof toWrite.featureFlags === "object" && !Array.isArray(toWrite.featureFlags)) {
        ...
        /* Same read-failure safety as users: this is a per-login MERGE against the current value. If
